...
export class SellerRepository extends DBOperations {
	constructor() {
		super();
	}
	...
	async updateAddress(input: AddressInput & { user_id: number }) {
		...
		let queryString =
			"INSERT INTO address(address_line1, address_line2, city, post_code, country, user_id) VALUES($1, $2, $3, $4, $5, $6)";
		const values = [
			input.addressLine1,
			input.addressLine2,
			input.city,
		...
		return await this.executeQuery(queryString, values);
	}
	async createPaymentMethod({
		...
		const queryString =
			"INSERT INTO payment_methods(bank_account, swift_code, payment_type, user_id) VALUES($1, $2, $3, $4)";
		const values = [Number(bankAccountNumber), swiftCode, paymentType, user_id];
		const result = await this.executeQuery(queryString, values);
		return result.rowCount > 0;
	}

	async getPaymentMethods(user_id: number) {
		const queryString = "SELECT * FROM payment_methods WHERE user_id=$1";
		const result = await this.executeQuery(queryString, [user_id]);
	...
	async updatePaymentMethod({
		...
		const queryString =
			"UPDATE payment_methods SET bank_account=$1, swift_code=$2, payment_type=$3 WHERE id=$4 AND user_id=$5";
		const values = [
